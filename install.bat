...
:: Check that Python version is 3.11 or 3.12 (pygame has no wheels for 3.13+ yet)
...
if %PYMAJ% neq 3 (
    echo ERROR: Python 3.11 or 3.12 is required. Detected Python %PYVER%.
...
if %PYMIN% gtr 12 (
    ...
    echo pygame does not provide pre-built wheels for Python 3.13 or newer.
...
if %errorlevel% neq 0 (
    ...
    echo If pygame failed to install, ensure you are using Python 3.11 or 3.12.
    echo Download Python 3.12 from https://www.python.org/downloads/
    pause
    exit /b 1
)
...
echo.
echo NOTE: If you see install errors, ensure you are using Python 3.11 or 3.12.
echo Download Python 3.12 from https://www.python.org/downloads/
echo.
pause
